...
def precompute_theta_pos_frequencies(head_dim: int, seq_len: int, device: str, theta: float=10000.0):
    assert head_dim % 2 == 0, "embedding dim must be even(divisible by 2) as written in the paper" 
    #Build the theta parameters
    #Acc to the formula, theta_i = 10000^(-2(i-1)/dim) for i = [1,2,...dim/2]
    # Shape = (Head_dim / 2)
    ...
    # we ca compute complex numbers in the polar form c = R * exp(1 * m * theta), where R = 1 as follows:
...
def apply_rotary_embeddings(x: torch.Tensor, freqs_complex: torch.Tensor, device:str):
    ...
    x_complex = torch.view_as_complex(x.float().reshape(*x.shape[:-1],-1,2))    #this op takes two consectutive dims and groups them, then transform this tensor to a complex tensor.
...
class EncoderBlock(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        # normalization bfore the attention block
        self.attention_norm = RMSNorm(args.dim, eps=args.norm_eps)
        # normalization bfore the feed forward block
...
class SelfAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_complex: torch.Tensor):
        ...
        # doesnt change the shape of the tensrs
        xq = apply_rotary_embeddings(xq, freqs_complex, device=x.device)
        xk = apply_rotary_embeddings(xk, freqs_complex, device=x.device)

        # replace the entry in the cachce for this token
        self.cache_k[:batch_size, start_pos : start_pos + seq_len] = xk
        self.cache_v[:batch_size, start_pos : start_pos + seq_len] = xv

        # retreive all the keys and values so far
...
class Transformer (nn.Module):
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        #Consecutively apply all the ecnoder blocks
